...
var IntervalTree = require('@flatten-js/interval-tree');
var unofficialGrammarlyApi = require('@emacs-grammarly/unofficial-grammarly-api');
...
const DEFAULT = {
    ...
    ignore: [],
    severity: {
        Determiners: vscodeLanguageserver.DiagnosticSeverity.Error,
        Misspelled: vscodeLanguageserver.DiagnosticSeverity.Error,
        Unknown: vscodeLanguageserver.DiagnosticSeverity.Error,
        ClosingPunct: vscodeLanguageserver.DiagnosticSeverity.Error,
        Nouns: vscodeLanguageserver.DiagnosticSeverity.Error,
        OddWords: vscodeLanguageserver.DiagnosticSeverity.Warning,
        CompPunct: vscodeLanguageserver.DiagnosticSeverity.Warning,
        Clarity: vscodeLanguageserver.DiagnosticSeverity.Warning,
        Dialects: vscodeLanguageserver.DiagnosticSeverity.Warning,
        WordChoice: vscodeLanguageserver.DiagnosticSeverity.Information,
        Readability: vscodeLanguageserver.DiagnosticSeverity.Information,
        PassiveVoice: vscodeLanguageserver.DiagnosticSeverity.Hint,
        _default: vscodeLanguageserver.DiagnosticSeverity.Hint,
    },
    userWords: [],
    diagnostics: {
        '[markdown]': {
...
let ConfigurationService = class ConfigurationService {
    ...
    register() {
        ...
        return vscodeLanguageserver.Disposable.create(() => {
            this.wip.clear();
            this.perDocumentSettings.clear();
        });
    }
    ...
    getDocumentSettings(uri, fresh = false) {
        ...
        const get = async () => {
            ...
            const override = config.overrides.find((override) => toArray(override.files).some((pattern) => minimatch__default['default'](uri, pattern)));
            const settings = {
    ...
    isIgnoredDocument(uri) {
        return toArray(this.user.ignore).some((pattern) => minimatch__default['default'](uri, pattern));
    }
};
...
const parser = remark__default['default']();
function parse(content) {
    const ast = parser.parse(content);
    const tree = new IntervalTree__default['default']();
    iterate(ast, node => {
...
class GrammarlyDocument {
    ...
    static create(uri, languageId, version, content) {
        return new GrammarlyDocument(vscodeLanguageserverTextdocument.TextDocument.create(uri, languageId, version, content));
    }
...
class TextGrammarCheckHost {
    constructor(clientInfo, document, getDocumentContext, getTokenInfo, onError) {
        ...
        this.events = new events.EventEmitter({ captureRejections: true });
        this.auth = null;
        this.revision = unofficialGrammarlyApi.getIdRevision(-1);
        this.localText = '';
        this.remoteText = '';
        this.alerts = reactivity.ref(new Map());
        this.user = reactivity.ref({ isAnonymous: true, isPremium: false, username: 'anonymous' });
        this.score = reactivity.ref(-1);
        this.generalScore = reactivity.ref(-1);
        this.status = reactivity.ref('CHECKING');
        this.scores = reactivity.ref({});
        this.emotions = reactivity.ref([]);
        this.textInfo = reactivity.ref(null);
        this.disposables = [];
        ...
        this.api = new unofficialGrammarlyApi.GrammarlyClient({
            clientName: name,
            clientType: type,
            ...
            onError: (error) => {
                this.onError(error);
                if (error instanceof unofficialGrammarlyApi.SocketError) {
                    if (error.code === 4000) {
                        this.api.ping();
                    }
                }
        ...
        this.on(unofficialGrammarlyApi.ResponseKind.ALERT, (alert) => {
            var _a;
            const changeset = new unofficialGrammarlyApi.ChangeSet(this.remoteText, this.localText);
            ...
            this.alerts.value.set(alert.id, reactivity.markRaw({ ...newAlert, raw: alert }));
        });
        this.on(unofficialGrammarlyApi.ResponseKind.REMOVE, (alert) => this.alerts.value.delete(alert.id));
        this.on(unofficialGrammarlyApi.ResponseKind.EMOTIONS, (message) => {
            this.emotions.value = message.emotions;
        });
        this.on(unofficialGrammarlyApi.ResponseKind.FINISHED, (message) => {
        ...
        this.on(unofficialGrammarlyApi.ResponseKind.TEXT_INFO, (message) => {
    ...
    setText(text) {
        this.repositionAlerts(new unofficialGrammarlyApi.ChangeSet(this.localText, text));
        this.localText = text;
        void this.sync();
    }
    repositionAlerts(changeset) {
        ...
        this.alerts.value.forEach((alert) => {
            ...
            newAlerts.set(newAlert.id, reactivity.markRaw({ ...newAlert, raw: alert.raw }));
        });
    ...
    async sync() {
        ...
        const changeset = new unofficialGrammarlyApi.ChangeSet(remoteText, localText);
        const deltas = changeset.diff();
        const rev = unofficialGrammarlyApi.getIdRevision(this.revision + 1);
...
class GrammarlyHostFactory {
    ...
    create(document, clientInfo) {
        ...
        const host = new TextGrammarCheckHost((_a = knownClients[clientInfo.name]) !== null && _a !== void 0 ? _a : clientInfo, document, () => this.getDocumentContext(document), () => this.getAuth(), (error) => {
            if (error instanceof unofficialGrammarlyApi.SocketError) {
                if (error.code === unofficialGrammarlyApi.SocketErrorCode.UNAUTHORIZED) {
                    this.auth = null;
    ...
    async asAnonymous() {
        return (this.auth = await unofficialGrammarlyApi.anonymous());
    }
    async asUser() {
        ...
        if (typeof credentials === 'string') {
            ...
            this.auth = await unofficialGrammarlyApi.authenticate(credentials.username, credentials.password);
            if (this.auth)
...
exports.GrammarlyLanguageServer = void 0;
...
})(exports.GrammarlyLanguageServer || (exports.GrammarlyLanguageServer = {}));
...
let DocumentService = DocumentService_1 = class DocumentService {
    constructor(connection, capabilities, client, configuration) {
        ...
        this.documents = new vscodeLanguageserver.TextDocuments(GrammarlyDocument);
        this.hostFactory = new GrammarlyHostFactory(async (document) => this.configuration.getDocumentSettings(document.uri), async () => this.getCredentials(), async (token) => this.connection.sendRequest(exports.GrammarlyLanguageServer.Client.Feature.storeToken, { token }));
    ...
    register() {
        ...
        const disposables = [
            ...
            vscodeLanguageserver.Disposable.create(() => this.documents.all().forEach((document) => document.detachHost())),
        ];
        return vscodeLanguageserver.Disposable.create(() => disposables.forEach((disposable) => disposable.dispose()));
    }
    get(uri) {
    ...
    async getCredentials() {
        try {
            const result = await this.connection.sendRequest(exports.GrammarlyLanguageServer.Client.Feature.getToken);
            if (result)
        ...
        return this.connection.sendRequest(exports.GrammarlyLanguageServer.Client.Feature.getCredentials);
    }
...
DocumentService = DocumentService_1 = __decorate([
    ...
    __param(2, inversify.inject(CLIENT_INFO)),
...
function watchEffect(cb) {
    const runner = reactivity.effect(cb, { lazy: true });
    runner();
    return () => {
        reactivity.stop(runner);
    };
...
let GrammarlyDiagnosticsService = GrammarlyDiagnosticsService_1 = class GrammarlyDiagnosticsService {
    ...
    register() {
        ...
        this.connection.onRequest(exports.GrammarlyLanguageServer.Feature.checkGrammar, (ref) => {
            this.LOGGER.trace(`${exports.GrammarlyLanguageServer.Feature.checkGrammar}(${JSON.stringify(ref, null, 2)})`);
            this.check(ref.uri);
        });
        this.connection.onRequest(exports.GrammarlyLanguageServer.Feature.stop, (ref) => {
            this.LOGGER.trace(`${exports.GrammarlyLanguageServer.Feature.stop}(${JSON.stringify(ref, null, 2)})`);
            const document = this.documents.get(ref.uri);
            if (document != null) {
                this.clearDiagnostics(document);
                this.connection.sendRequest(exports.GrammarlyLanguageServer.Client.Feature.updateDocumentState, { uri: document.uri });
                document.detachHost();
            }
        });
        this.connection.onRequest(exports.GrammarlyLanguageServer.Feature.getDocumentState, (ref) => {
            this.LOGGER.trace(`${exports.GrammarlyLanguageServer.Feature.checkGrammar}(${JSON.stringify(ref, null, 2)})`);
            const document = this.documents.get(ref.uri);
        ...
        this.connection.onRequest(exports.GrammarlyLanguageServer.Feature.acceptAlert, (options) => {
            var _a;
            this.LOGGER.trace(`${exports.GrammarlyLanguageServer.Feature.acceptAlert}(${JSON.stringify(options, null, 2)})`);
        ...
        this.connection.onRequest(exports.GrammarlyLanguageServer.Feature.dismissAlert, (options) => {
            var _a;
            this.LOGGER.trace(`${exports.GrammarlyLanguageServer.Feature.dismissAlert}(${JSON.stringify(options, null, 2)})`);
            const document = this.documents.get(options.uri);
            (_a = this.diagnostics.get(options.uri)) === null || _a === void 0 ? void 0 : _a.delete(options.id);
            if (document === null || document === void 0 ? void 0 : document.host) {
                ...
                if (alert) {
                        ...
                        alert.replacements.forEach((newText, index) => {
                            actions.push({
                                ...
                                command: {
                                    ...
                                    arguments: [
                                        {
                                            method: exports.GrammarlyLanguageServer.Feature.acceptAlert,
                                ...
                                edit: {
                                    changes: {
                                        [textDocument.uri]: subAlerts.map(subAlert => {
                                            ...
                                            const newText = unofficialGrammarlyApi.applyDelta(oldText, subAlert.transformJson.alternatives[i]);
                    ...
                    actions.push({
                        ...
                        command: {
                            ...
                            arguments: [
                                {
                                    method: exports.GrammarlyLanguageServer.Feature.dismissAlert,
                                    params: { id: alert.id, uri: textDocument.uri },
                                },
                            ],
                        },
    ...
    setupDiagnostics(document) {
        ...
        document.host.on(unofficialGrammarlyApi.ResponseKind.FINISHED, () => {
            this.sendDiagnostics(document);
        });
        this.sendDiagnostics(document);
    }
    sendDocumentState(document) {
        this.connection.sendRequest(exports.GrammarlyLanguageServer.Client.Feature.updateDocumentState, this.getDocumentState(document));
    }
    ...
    toDiagnostics(alert, document) {
        ...
        if (isNumber(alert.begin) && isNumber(alert.end)) {
            ...
            diagnostics.push({
                ...
                source: SOURCE,
                severity: severity,
                tags: severity === vscodeLanguageserver.DiagnosticSeverity.Hint ? [vscodeLanguageserver.DiagnosticTag.Unnecessary] : [],
                rev: alert.rev,
