...
async function anonymous(client, version) {
    if (process.env.NODE_ENV !== 'production')
    ...
    const response = await fetch('https://auth.grammarly.com/v3/user/oranonymous?app=chromeExt&containerId=' + cookie.parsed.gnar_containerId, {
        method: 'GET',
        headers: {
            ...BROWSER_HEADERS,
            Accept: 'application/json',
            'x-client-type': client !== null && client !== void 0 ? client : 'extension-chrome',
            'x-client-version': version !== null && version !== void 0 ? version : '1.2.390-SNAPSHOT',
...
var name = "@emacs-grammarly/unofficial-grammarly-api";
var version = "0.2.0";
...
const UA$1 = `${name} v${version} (NodeJS ${process.version})`;
class SocketClient {
    constructor(id, _getToken, _onConnection = () => { }, _onMessage = () => { }, ua = '', additionalHeaders = {}) {
        this.id = id;
        this._getToken = _getToken;
        this._onConnection = _onConnection;
        this._onMessage = _onMessage;
        this.additionalHeaders = additionalHeaders;
        this.LOGGER = process.env.NODE_ENV !== 'production' ? new DevLogger(SocketClient.name, this.id.substr(0, 6)) : null;
        this._socket = null;
    ...
    async _connect() {
        ...
        if (process.env.NODE_ENV !== 'production')
            (_e = this.LOGGER) === null || _e === void 0 ? void 0 : _e.trace('Send Headers: ', headers);
        this._socket.onopen = () => {
            var _a;
            if (process.env.NODE_ENV !== 'production')
    ...
    _queueOrSend(message, priority) {
        var _a;
        if (process.env.NODE_ENV !== 'production')
            (_a = this.LOGGER) === null || _a === void 0 ? void 0 : _a.trace(`WebSocket: Send(${message.id}): ${message.action}`);
        if (!this._socket) {
            this._connect();
    ...
    _sendToSocket(message) {
        ...
        if (process.env.NODE_ENV !== 'production')
            (_a = this.LOGGER) === null || _a === void 0 ? void 0 : _a.trace(`CAPI: Send(${message.id}) — ${message.action}`, message);
    ...
    _handleMessage(response) {
        ...
        if (isAckResponse(response)) {
            if (process.env.NODE_ENV !== 'production')
                (_a = this.LOGGER) === null || _a === void 0 ? void 0 : _a.trace(`CAPI: Ack(${response.id}) — ${response.action}`, response);
            this._pendingResponses -= 1;
            ...
            if (process.env.NODE_ENV !== 'production')
                (_b = this.LOGGER) === null || _b === void 0 ? void 0 : _b.trace(`CAPI: Recv() — ${response.action}`, response);
        }
        try {
...
function applyDelta(text, change) {
    let newText = '';
...
export { ChangeSet, GrammarlyClient, RequestKind, ResponseKind, SocketClient, SocketError, SocketErrorCode, anonymous, applyDelta, authenticate, checkGrammar, checkPlagiarism, createCheckClient, getIdRevision, isAckResponse, isEvent, isRequestType, isResponseType };
